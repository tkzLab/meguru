...
const CONF = {
   ...
   */
  /**
   * 【段10（2026-08-16）葉脈の向きと規則性を作り直した】§11.21。
   *
   * 段9 で膜が金色になり、段4 の葉脈が**幾何学的なヘリンボーン（山形）**として前に出た。
   * 実装の形に合わせた参照画像（`work/moodboard-flower-bowl.png` / `-side.png`）と
   * 同じ縮尺で測ったところ、**本数も間隔もほぼ一致していて、違うのは規則正しさだけ**だった:
   *
   *   稜の本数  参照 42〜44 / 実装 41       間隔  参照 2.13〜2.20% / 実装 2.07%
   *   隣接比    参照 0.956〜1.021 / 実装 **0.105**   ← 隣り合う間隔がほぼ完全に等しい
   *   横切る成分 参照 17.8〜18.4% / 実装 **5.9%**    ← 参照の枝分かれが作っている
   *
   * 旧式 `phase = vU*Count - |vV|*Slope` は等位線が放射方向を**横切る** V 字で、
   ...
   * 光粒 1 個が膜を照らす強さ。
   *
   * 【段11（2026-08-16）0.30 → 0.14】§11.22。
   * 段9（花の自己発光）と段10（葉脈を発光として足した）で、**§11.3 の明るさ予算を
   * 門が無いまま超えていた**（ムードボードと同条件で 輝度70以上 14.2〜15.5% / 上限10、
   * 輝度200以上 1.6〜2.3% / 上限1.0）。白飛びはしていなかったので検査は全部緑だった。
   * 予算内へ戻す判断（ユーザー決定）。ムードボードの 6.12% に対し 5.6〜6.1% に着地。
   */
  flowerGlowGain: 0.14,
  ...
  return new THREE.ShaderMaterial({
    ...
    vertexShader: /* glsl */ `
      ...
      void main() {
        vU = aU;
        vV = aV;
        // 葉脈の位相をずらす種。**花弁ごと（aPh）と花ごと（個体差 h1）で違う値にする。**
        // 同じ種だと 7 枚 × 13 輪が全部同じ模様になり、不規則にしたつもりが
        // 「同じ不規則」の繰り返しとして目に付く（段10）
        vSeed = aPh;
        // instanceColor の R に「蓄えの割合」が入っている
        #ifdef USE_INSTANCING_COLOR
          vCharge = instanceColor.r;
          vSeed += instanceColor.g * 6.2831;
        #else
          vCharge = 1.0;
        #endif
    ...
    fragmentShader: /* glsl */ `
      ...
      void main() {
        // 膜の縁。
        //
        // 【この注意書きは段12（§11.23）で条件付きになった】
        // もとは「ここに平坦部を作ってはいけない。途中で 1.0 に達する形にすると
        // 輪郭線が立って板に見える（ガラスの破片のような直線が出た）」だった。
        // **それは 1輪が 30px しかなかった頃の話。** 段7 で camDistance を 48 → 24 に
        // 寄せて手前の花が 1483px になった今は、逆に**縁が無いことの方が不自然に出る**
        // （リムを4通り試して全部「弧」や「板」になり、side を3倍急にしても
        //   画面で測った縁の幅が動かなかった ＝ 形に「端」が無いのが原因だった）。
        // いまは膜の形をテクスチャ（src/petal.js）が持ち、**縁の直前まで不透明度を保つ**。
        // 硬くしすぎる側は test/petal.test.mjs が縁の幅 1.2〜3.2% で見ている。
        ...
        // 【段10: 平均0の変調をやめ、葉脈を発光として足す】§11.21。
        // 段4 の目的は「総光量を増やさずに葉脈を見せる」だったが、段9 で花の役割が変わり、
        // いまは**花が吸った光で自ら光り、その光が葉脈に沿う**のが目的。
        // 平均0の掛け算では「暗い膜の上に載った明るい金の繊維」は原理的に作れない。
